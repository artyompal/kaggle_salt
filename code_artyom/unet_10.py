...
save_model_name = basic_name + '_stage1_epoch{epoch:02d}_iou{my_iou_metric:.4f}.hdf5'
...
save_model_name = basic_name + '_stage2_epoch{epoch:02d}_iou{val_my_iou_metric_2:.4f}.hdf5'
...
names = list(glob(basic_name + '_stage2*.hdf5'))
names.sort(key=parse_iou)
assert(len(names) > 0)
save_model_name = names[-1]
