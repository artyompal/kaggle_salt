...
import os, pickle, random, sys, subprocess
...
def train(x_train: NpArray, x_valid: NpArray, depth_train: NpArray,
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric', mode='max', factor=0.3, patience=5, min_lr=3e-6, verbose=1)
...
def predict(model_name: str, x_valid: NpArray, x_test: NpArray,
...
if __name__ == "__main__":
    ...
    if not ENABLE_KFOLD:
        ...
        preds_valid, preds_test[0] = predict(model_name, x_valid, x_test)
        ...
        for fold, indices in enumerate(folds.split(images, labels_for_strat)):
            ...
            p_val, p_test = predict(model_name, x_valid, x_test, fold)
