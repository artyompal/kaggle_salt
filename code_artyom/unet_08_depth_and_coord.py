...
# Build model
def build_model(input_layer, depth_inp_layer, start_neurons, DropoutRatio=0.5):
    # 101 -> 50
    ...
    pool4 = concatenate([pool4, depth_inp_layer])
    # pool4 = Add()([pool4, depth_inp_layer])
...
def train(x_train: NpArray, x_valid: NpArray, depth_train: NpArray,
          depth_valid: NpArray, y_train: NpArray, y_valid: NpArray,
          fold: int = -1) -> str:
    input_layer = Input((img_size_target, img_size_target, 3))
    depth_inp_layer = Input((6, 6, 1))
    output_layer = build_model(input_layer, depth_inp_layer, 16, 0.5)

    model = Model(inputs=[input_layer, depth_inp_layer], outputs=output_layer)

    learning_rate = 1e-2
    opt = optimizers.adam(lr=learning_rate)
    model.compile(loss="binary_crossentropy", optimizer=opt, metrics=[my_iou_metric])
    # model.summary()

    stage1_model_name = make_output_path("models/stage1_fold%d_epoch{epoch:02d}_iou{val_my_iou_metric:.4f}.hdf5" % fold)
    model_checkpoint = ModelCheckpoint(stage1_model_name, monitor='val_my_iou_metric',
                                       mode='max', save_best_only=True, verbose=1)
    early_stopping = EarlyStopping(monitor='val_my_iou_metric', mode='max', patience=10, verbose=1)
    reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric', mode='max', factor=0.1, patience=5, min_lr=3e-6, verbose=1)

    epochs = EPOCHS
    batch_size = 32
    history = model.fit([x_train, depth_train], y_train,
                        validation_data=[[x_valid, depth_valid], y_valid],
                        epochs=epochs,
                        batch_size=batch_size,
                        callbacks=[early_stopping, model_checkpoint, reduce_lr],
                        verbose=2)

    names = list(glob(make_output_path("models/stage1_fold%d_epoch*.hdf5" % fold)))
    names.sort(key=parse_iou)
    assert(len(names) > 0)
    stage1_model_name = names[-1]
    print("stage1 model found", stage1_model_name)
    model = load_model(stage1_model_name, custom_objects={'my_iou_metric': my_iou_metric})
    ...
    stage2_model_name = make_output_path("models/stage2_fold%d_epoch{epoch:02d}_iou{val_my_iou_metric_2:.4f}.hdf5" % fold)
    model_checkpoint = ModelCheckpoint(stage2_model_name, monitor='val_my_iou_metric_2',
                                       mode='max', save_best_only=True, verbose=1)
    early_stopping = EarlyStopping(monitor='val_my_iou_metric_2', mode='max', patience=20, verbose=1)
    reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric_2', mode='max', factor=0.3, patience=5, min_lr=3e-6, verbose=1)
    epochs = EPOCHS
    batch_size = 32
    ...
    names = list(glob(make_output_path("models/stage2_fold%d_epoch*.hdf5" % fold)))
    names.sort(key=parse_iou)
    assert(len(names) > 0)
    stage2_model_name = names[-1]
    print("stage2 model found", stage2_model_name)
    return stage2_model_name

def predict_result(model, x_test, depth_test):
    """ Predict both orginal and reflected x """
...
def predict(model_name: str, x_valid: NpArray, x_test: NpArray, depth_test: NpArray,
            fold: int = -1) -> Tuple[NpArray, NpArray]:
    model = load_model(model_name,
                       custom_objects={'my_iou_metric_2': my_iou_metric_2,
                                       'lovasz_loss': lovasz_loss})

    preds_valid = predict_result(model, x_valid, depth_test)
    preds_test = predict_result(model, x_test, depth_test)
...
def make_depth(depth: NpArray) -> NpArray:
    """ Takes dataset depth (N, 1), returns (N, 6, 6, 1). """
    depth = (depth - np.min(depth)) / (np.max(depth) - np.min(depth))
    depth = depth.reshape((-1, 1, 1, 1))
    res = np.zeros((depth.shape[0], 6, 6, 1))
    res[...] = depth[...]
    print("make_depth", res.shape)
    return res
...
if __name__ == "__main__":
    ...
    depths = make_depth(train_df.iloc[:, 0].values)
    ...
    depth_test = make_depth(test_df.iloc[:, 0].values)
    ...
    if not ENABLE_KFOLD:
        ...
        preds_valid, preds_test[0] = predict(model_name, x_valid, x_test, depth_test)
        ...
        for fold, indices in enumerate(folds.split(images, labels_for_strat)):
            ...
            depth_train, depth_valid = depths[train_idx], depths[valid_idx]
            x_valid, y_valid = images[valid_idx], masks[valid_idx]

            # data augmentation
            x_train = np.append(x_train, [np.fliplr(x) for x in x_train], axis=0)
            y_train = np.append(y_train, [np.fliplr(x) for x in y_train], axis=0)
            depth_train = np.append(depth_train, [np.fliplr(x) for x in depth_train], axis=0)

            model_name = train(x_train, x_valid, depth_train, depth_valid,
                               y_train, y_valid, fold)
            p_val, p_test = predict(model_name, x_valid, x_test, depth_test, fold)
