...
class Datagen(keras.utils.Sequence):
    ...
    def __init__(self, x: NpArray, y: NpArray, valid: bool) -> None:
        ...
        self.augs = albu.Compose([
            # albu.Compose([
            #     albu.RandomSizedCrop(min_max_height=(SZ//2, SZ), height=SZ, width=SZ, p=1),
            #     albu.PadIfNeeded(min_height=SZ, min_width=SZ, p=1),
            # ], p=0.5),

            albu.ShiftScaleRotate(rotate_limit=10, p=0.5),
...
def train(x_train: NpArray, x_valid: NpArray, y_train: NpArray, y_valid: NpArray,
    ...
    model.compile(opt, 'binary_crossentropy', metrics=[my_iou_metric])

    model_name = make_output_path("models/fold%d_epoch{epoch:02d}_iou{val_my_iou_metric:.4f}.hdf5" % fold)
